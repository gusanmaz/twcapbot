...
func (b * TweetCaptionBot) CaptionTweet(id int64, rootPath string) error{
	...
	fNameInfo :=  GenerateFileNamesForTweet(tw)
	twUserDirName := fmt.Sprintf("%v_%v", tw.User.ScreenName, tw.User.Id)
	userDirPath := filepath.Join(rootPath, twUserDirName)
	...
	for _, v := range fNameInfo{
		...
		if v.MediaTweet{
			...
			err = capdec.Caption(srcPath, GetCaptionsForTweet(tw),destFilePath, b.JSCodes)
			if err != nil{
			...
			err := capdec.Caption(b.HairPhotoPath, GetCaptionsForTweet(tw),destFilePath,b.JSCodes)
			if err != nil{
				b.ErrLog.Printf("Captioning of tweet with ID of %v is unsuccessful!", tw.Id)
				b.ErrLog.Printf("Error message: %v", err)
				panic("Exiting program...")
	...
	urlFileName  := fmt.Sprintf("%v.url", tw.Id)
	urlFilePath := filepath.Join(userDirPath, urlFileName)
	urlFile, err := os.OpenFile(urlFilePath, os.O_RDWR|os.O_CREATE, 0644)
	defer urlFile.Close()
	if err != nil{
		b.ErrLog.Printf("Couldn't create %v", tw.Id)
	}
	_, err = urlFile.WriteString(GetTweetURL(tw))
	if err != nil{
		b.ErrLog.Printf("Couldn't write tweet (ID: %v) url into %v", tw.Id, urlFilePath)
	}
